...
IMEM thisIMEM(.Address(PC),.InstBits(IBits_temp),.Ready(IMEM_READY),.clk(clk));
assign IBits = (~IMEM_READY)?NOPbits:IBits_temp;
...
DMEM thisDMEM(.Address(MEM_ALUOut),.SValue(MEM_StoreValue),.RValue(MemOut_temp),.ReadMem(MEM_memToReg_ctrl),.WriteMem(MEM_memWrite_ctrl),.LBU(MEM_LBU_ctrl),.ByteOp(MEM_ByteSize_ctrl),.clk(clk),.Ready(DMEM_READY));
assign MemOut = (~DMEM_READY)?NOPbits:MemOut_temp; // Assign NOPbits to output if DMEM is not ready
